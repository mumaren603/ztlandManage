...
class sqlQuery():
    ...
    # 商品房首次批量
    def getHouseFirstPlRegisterData(self):
        querySQL=" select bdcdyh,zddm,mph as zl from dc_z where bdcdyh in" \
                 "(select a.lszbdcdyh from " \
                 "(select fwbh,lszfwbh,lszbdcdyh from dc_h_fwzk where zt = '1' and sfyx = '0'  and fwyt1 != '84' and fwyt1 != '54' and fwyt1 != '22'  and fwyt1 != '8401')a " \
                 "left join(select fwbh from dc_h  where zt='1')b on a.fwbh = b.fwbh " \
                 "left join (select fwbh,zddm,mph from dc_z where zt='1' )c  on a.lszfwbh = c.fwbh " \
                 "left join (select zddm from dc_djdcbxx where zt='1')d on c.zddm = d.zddm " \
                 "having count(lszbdcdyh) > 30 and count(lszbdcdyh) <100 group by lszbdcdyh) order by dbms_random.value()"
        queryRes = self.db_qj_conn.SqlExecute(querySQL)
        print("权籍查询数据：" ,queryRes)
        if queryRes[0]:
            return queryRes
    ...
    def getZjfwFirstRegisterData(self):
        ...
        if queryRes[0]:
            ...
            if querySqxxSQLRes[0] or querySqxxzbSQLRes[0]:
                print("该数据已在办理中，重新获取数据！")
                return sqlQuery(self.env).getZjfwFirstRegisterData()
            else:
                print("数据符合！数据为：", queryRes)
                self.db_qj_conn.closeConn()
                self.db_dj_conn.closeConn()
    ...
    def getGjptFirstRegisterData(self):
        querySQL="select distinct d.bdcdyh,d.zddm,d.tdzl as zl from （select lszfwbh,bdcdyh,fwbh,zl from kjk.dc_h_fwzk where zt = '1' and sfyx = '0' and (fwlx = '9901' or fwlx = '99')) a left join (select fwbh from kjk.dc_h where zt = '1' and bdcdyh>'0') b on a.fwbh = b.fwbh left join （select fwbh,zddm from kjk.dc_z where zt = '1') c on a.lszfwbh = c.fwbh left join (select zddm,bdcdyh,tdzl from kjk.dc_djdcbxx where zt = '1' and sfyx = '1') d on c.zddm = d.zddm where d.bdcdyh > '0' and rownum <50  order by dbms_random.value()"
        queryRes = self.db_qj_conn.SqlExecute(querySQL)
        print("权籍查询数据为：" , queryRes)
        if queryRes[0]:
            self.db_qj_conn.closeConn()
    ...
    # 公建配套转移、变更登记数据
    def getGjptChangeRegisterData(self):
        querySQL="select bdcdyh,fwdm,zl from (select bdcdyh,fwdm,zl,cqbid from djjgk.dj_fsssxx t where zt='1' and sfyx=1)a inner join (select id from djjgk.dj_jzwqfsyqyzgybf t where zt='1' and sfyx=1)b on a.cqbid = b.id where bdcdyh > '0' and rownum < 50 order by dbms_random.value()"
        queryRes = self.db_dj_conn.SqlExecute(querySQL)
        print("登记查询数据为：" , queryRes)
        if queryRes[0]:
            # 检查该数据是否存在待办件
            querySqxxSQL = "select count(1) from ywbdk.yw_sqxx where ajzt='1' and sfyx=1 and bdcdyh='" + queryRes[0] + "'"
            querySqxxzbSQL = "select count(1) from YWBDK.yw_sqxx t where t.SFYX = '1' and t.ajzt in ('1', '3', '6') and t.id in (select z.sqbid from ywbdk.yw_sqxxzb z where z.sfyx = '1' and bdcdyh = '" +queryRes[0] + "')"
            ...
            if querySqxxSQLRes[0] or querySqxxzbSQLRes[0]:
                print("该数据已在办理中，重新获取数据！")
                return sqlQuery(self.env).getGjptChangeRegisterData()
            else:
                print("数据符合！数据为：", queryRes)
                self.db_qj_conn.closeConn()
                self.db_dj_conn.closeConn()
    ...
    def getCkFirstRegisterData(self):
        querySQL="select c.bdcdyh,c.zddm,c.mph as zl from " \
                 ...
                 "where 1 = 1 and (a.FWLX = '6' or a.FWLX = '9902' or a.FWLX = '99') and c.bdcdyh > '0' and  rownum<50 order by dbms_random.value()"
        queryRes = self.db_qj_conn.SqlExecute(querySQL)
        print("权籍查询数据为：" , queryRes)
        if queryRes[0]:
            self.db_qj_conn.closeConn()
    ...
    # 车库位转移登记数据
    def getCkChangeRegisterData(self):
        querySQL="select distinct a.bdcdyh,a.fwdm,a.zl from (select bdcdyh,fwdm,zl from djjgk.dj_fsss t where zt='1' and sfyx=1)a " \
                 "inner join (select bdcdyh from djjgk.dj_fsssxx t where zt='1' and sfyx=1)b on a.bdcdyh = b.bdcdyh where a.bdcdyh > '0' " \
                 "and rownum < 50 order by dbms_random.value()"
        queryRes = self.db_dj_conn.SqlExecute(querySQL)
        print("登记查询数据为：", queryRes)
        if queryRes[0]:
            ...
            if querySqxxSQLRes[0] or querySqxxzbSQLRes[0]:
                print("该数据已在办理中，重新获取数据！")
                return sqlQuery(self.env).getCkChangeRegisterData()
            else:
                print("数据符合！数据为：", queryRes)
                self.db_qj_conn.closeConn()
                self.db_dj_conn.closeConn()
