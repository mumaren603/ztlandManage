...
from common.validMode import envDetailInfoValidTemplate,addEnvValidTemplate,dbDetailInfoValidTemplate
import json
        ...
        if request.method == 'GET':
            try:
                ...
                models.EnvDetailInfo.objects.filter(s_id=nid).delete()
                    ...
                    print(e)
            return redirect('/env/detail-%s' %env_sub_node_id)
    else:
        return render(request,'login.html')
    ...
    if request.session.get('is_login',None):
        if request.method == 'GET':
            try:
                ...
                pass
                # print(e)  #后期写到日志
            return redirect('/env/detail-%s' % db_node_id)
    else:
        return render(request,'login.html')
